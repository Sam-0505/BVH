...
Mesh::Mesh(std::vector<Vec3> positions, std::vector<std::uint32_t> indices)
    ...
    for (std::size_t i = 0; i < indices_.size(); ++i) {
        ...
        if (static_cast<std::size_t>(indices_[i]) >= positions_.size()) {
...
void Mesh::recomputeBounds() {
    bounds_ = AABB{};
    for (std::uint32_t index : indices_) bounds_.extend(positions_[index]);

    vertexBounds_ = AABB{};
    for (const Vec3& p : positions_) vertexBounds_.extend(p);
}
